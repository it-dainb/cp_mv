    ...
    with torch.no_grad():
        for images, case_ids, original_sizes in tqdm(dataloader, desc="Inference"):
            ...
            for i in range(images.size(0)):
                ...
                case_id = case_ids[i]
                original_size = (
                    original_sizes[0][i].item(),
                    original_sizes[1][i].item(),
                )
                ...
                instances = extract_instances_from_mask(
                    pred_mask_binary, min_area=min_area
                )
...
def create_submission(predictions, output_path="submission.csv"):
    ...
    for case_id, instances in tqdm(predictions.items(), desc="Creating submission"):
        if len(instances) == 0:
            ...
            submission_data.append({"case_id": case_id, "annotation": rle_encoded})
...
def main(args):
    ...
    predictions = predict_instances(
        model, test_loader, device, threshold=args.threshold, min_area=args.min_area
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate predictions for forgery detection"
    )

    # Data parameters
    parser.add_argument(
        "--test-images", type=str, required=True, help="Path to test images directory"
    )
    parser.add_argument(
